...
+(instancetype)showPickerWithTitle:(NSString *)title initialSelection:(NSArray *)initialSelection doneBlock:(ActionDataDoneBlock)doneBlock cancelBlock:(ActionDataCancelBlock)cancelBlock origin:(UIView *)view
{
    ActionSheetDataPicker *picker = [[ActionSheetDataPicker alloc] initWithTitle:title initialSelection:initialSelection doneBlock:doneBlock cancelBlock:cancelBlock origin:view];
    
    return picker;
}

-(instancetype)initWithTitle:(NSString *)title initialSelection:(NSArray *)initialSelection doneBlock:(ActionDataDoneBlock)doneBlock cancelBlock:(ActionDataCancelBlock)cancelBlock origin:(UIView *)view
{
 ...
 *  @param value         值
 *  @param component     列
 *  @param orgDictionary 要查找的数据
 *  @param rowDictionary 该行的数据
 *
 *  @return 返回行
 */
 ...
 *  获取当选择前列当前行的数据对象
 *
 *  @param currentComponent 选择的当前列
 *  @param parentComponent  父一列(调用时，请传0)
 *  @param dictionary       数据
 *  @param row              选择的当前行
 *
 *  @return
    ...
    if ([value isKindOfClass:[NSString class]] || [value isKindOfClass:[NSNumber class]]) {
        ...
        if ([array count] > 0) {
            ...
            if ([firstDict isKindOfClass:[NSDictionary class]]) {
                //                NSDictionary *tmpDict = (NSDictionary *)firstDict;
                NSArray *tmpKeys = firstDict.allKeys;
                NSString *key = @"";
                if (tmpKeys.count > 0) {
                    key = tmpKeys[0];
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    NSLog(@"列：%ld, 行：%ld, 内容: %@", component, row, text);
